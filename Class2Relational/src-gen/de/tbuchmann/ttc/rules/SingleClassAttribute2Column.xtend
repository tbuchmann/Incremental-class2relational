...
abstract class SingleClassAttribute2Column extends Elem2Elem {
	...
	override CorrModelDelta sourceToTarget(Set<EObject> _detachedCorrElems) {
		...
		for (_match : _matches) {
			...
			val col = unwrap(_trg.get(0) as SingleElem) as relational_.Column
	...
	override CorrModelDelta targetToSource(Set<EObject> _detachedCorrElems) {
		...
		for (col : targetModel.allContents.filter(typeof(relational_.Column)).toIterable()) {
	...
	def protected abstract Type4colName colNameFrom(String attName, class_.Classifier attType);
	
	@Data protected static class Type4colType {
		relational_.Type colType
	}
	def protected abstract Type4colType colTypeFrom(String attName, class_.Classifier attType);
	...
	@Data protected static class Target {
		relational_.Column col
	}
	def protected static Target target(Corr _corr) {
		_corr.assertRuleId("SingleClassAttribute2Column")
		val col = unwrap(_corr.target.get(0)) as relational_.Column
