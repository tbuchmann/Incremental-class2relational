...
abstract class Class2Table extends Elem2Elem {
	...
	override CorrModelDelta sourceToTarget(Set<EObject> _detachedCorrElems) {
		...
		for (clz : sourceModel.allContents.filter(typeof(class_.Class)).toIterable()) {
		...
		for (_match : _matches) {
			...
			val tbl = unwrap(_trg.get(0) as SingleElem) as relational_.Table
			...
			val _attSinCol = clz.getAttr()
					...
					.map[unwrap(corr.target.get(0) as SingleElem) as relational_.Column]
					.toList()
			...
			val _attMulTbl = clz.getAttr()
					...
					.map[unwrap(corr.target.get(0) as SingleElem) as relational_.Table]
					.toList()
	...
	override void onTrgElemCreation(EObject trgElem) {
		switch (trgElem.corrElemPosition) {
			case 0: onTblCreation(trgElem as relational_.Table)
		}
	}
	def protected abstract void onTblCreation(relational_.Table tbl);
	...
	override CorrModelDelta targetToSource(Set<EObject> _detachedCorrElems) {
		...
		for (tbl : targetModel.allContents.filter(typeof(relational_.Table)).toIterable()) {
	...
	def protected static Source source(Corr _corr) {
		...
		val clz = unwrap(_corr.source.get(0)) as class_.Class
		return new Source(clz)
	}
	@Data protected static class Target {
		relational_.Table tbl
	}
	def protected static Target target(Corr _corr) {
		_corr.assertRuleId("Class2Table")
		val tbl = unwrap(_corr.target.get(0)) as relational_.Table
