...
class DataType2Type extends Elem2Elem {
	...
	override CorrModelDelta sourceToTarget(Set<EObject> _detachedCorrElems) {
		...
		for (dt : sourceModel.allContents.filter(typeof(class_.DataType)).toIterable()) {
		...
		for (_match : _matches) {
			...
			val t = unwrap(_trg.get(0) as SingleElem) as relational_.Type
	...
	override CorrModelDelta targetToSource(Set<EObject> _detachedCorrElems) {
		...
		for (t : targetModel.allContents.filter(typeof(relational_.Type)).toIterable()) {
	...
	def protected static Source source(Corr _corr) {
		...
		val dt = unwrap(_corr.source.get(0)) as class_.DataType
	...
	def protected static Target target(Corr _corr) {
		_corr.assertRuleId("DataType2Type")
		val t = unwrap(_corr.target.get(0)) as relational_.Type
		return new Target(t)
	}
}
